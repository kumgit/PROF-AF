...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, SINGLE_BUTTON, PROTECT_CONTENT, SPELL_CHECK_REPLY, IMDB_TEMPLATE, IMDB_DELET_TIME, START_MESSAGE, PMFILTER, G_FILTER, SHORT_URL, SHORT_API
...
async def pm_next_page(bot, query):
    ...
    if not search:
        await query.answer("If takes too much time, please send the request again.", show_alert=True)
        return
    ...
    btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}]\n{file.file_name}", callback_data=f'pmfile#{file.file_id}')] for file in files ]
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):        
        ...
        await query.answer("🤨 Curiosity is little more, isn't it? 😁", show_alert=False)
        ...
        buttons = [[
            ...
            InlineKeyboardButton("🎥 ᴍᴏᴠɪᴇ ᴜᴘᴅᴀᴛᴇs", url="https://t.me/ContenTeam")
            ],[      
        ...
        buttons = [[
            InlineKeyboardButton('SOURCE CODE', url='https://t.me/sstrangerrr')
            ],[
...
async def pm_AutoFilter(client, msg, pmspoll=False):
    ...
    if SINGLE_BUTTON:
        btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}]\n{file.file_name}", callback_data=f'{pre}#{file.file_id}')] for file in files]
    else:
